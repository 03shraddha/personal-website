    ...
    const experiencesHtml = CONTENT.experiences.map((exp, index) => `
        <article class="experience-item" data-index="${index}">
            <div class="experience-header">
                <h3><a href="${exp.companyUrl}" ${exp.companyUrl.startsWith('http') ? 'target="_blank"' : ''} class="highlight ${exp.highlight || 'peach'}">${exp.title}</a></h3>
...
async function loadSubstackPosts() {
    const CACHE_KEY = 'substack-posts-v8';
